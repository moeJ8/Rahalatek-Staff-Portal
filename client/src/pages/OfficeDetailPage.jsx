...
const OfficeDetailPage = () => {
  ...
  const fetchOfficeVouchers = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/office-payments/${encodeURIComponent(displayName)}/vouchers`,
        {
      ...
      console.log(
        `⚡ Loaded ${response.data.count} relevant vouchers for ${displayName}`
      );
  ...
  const downloadOfficeDetailPDF = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/office-payments/${encodeURIComponent(displayName)}/download-pdf`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: {
            year: filters.year,
  ...
  const calculateServicePayments = useCallback(
    (voucher) => {
      const services = {
        hotels: 0,
        transfers: 0,
      ...
      if (
        ...
        voucher.payments.trips.officeName === displayName
      ) {
  ...
  const serviceVouchers = useMemo(() => {
    ...
    return vouchers.filter((voucher) => {
      ...
      const hasHotelService = voucher.hotels?.some(
        (hotel) => hotel.officeName === displayName
      );
      const hasTransferService = voucher.transfers?.some(
        (transfer) => transfer.officeName === displayName
      );
      // Check for trip services - both individual and global
      let hasTripService = false;
      if (voucher.trips && Array.isArray(voucher.trips)) {
        hasTripService = voucher.trips.some(
          (trip) =>
            trip && typeof trip === "object" && trip.officeName === displayName
        );
      }
      if (!hasTripService) {
        hasTripService = voucher.payments?.trips?.officeName === displayName;
      }
      const hasFlightService = voucher.flights?.some(
        (flight) => flight.officeName === displayName
      );
      const hasOtherService = voucher.others?.some(
        (other) =>
          other && typeof other === "object" && other.officeName === displayName
      );
  ...
  const filteredVouchers = useMemo(() => {
    return serviceVouchers.filter((voucher) => {
      ...
      if (serviceTableSearch.trim()) {
        ...
        const voucherNumber =
          voucher.voucherNumber?.toString().toLowerCase() || "";
        const clientName = voucher.clientName?.toLowerCase() || "";

        if (
          !voucherNumber.includes(searchTerm) &&
  ...
  const getClientVouchers = useMemo(() => {
    ...
    return vouchers.filter((voucher) => {
      ...
      if (isDirectClient) {
        if (voucher.officeName || voucher.clientName !== displayName)
          return false;
      } else {
  ...
  const calculateClientVoucherDetails = useCallback(
    (voucher) => {
      if (!voucherPayments)
        return { totalPaid: 0, remaining: voucher.totalAmount };
      ...
      const voucherSpecificPayments = voucherPayments.filter((payment) => {
        ...
        return (
          relatedVoucherId === voucher._id && payment.status === "approved"
        );
      });
  ...
  const calculateVoucherRemaining = useCallback(
    (voucherId, servicesTotal) => {
  ...
  const totals = useMemo(() => {
    const baseTotal = filteredVouchers.reduce(
      (acc, voucher) => {
    ...
    const clientTotals = getClientVouchers.reduce(
      (acc, voucher) => {
    ...
    filteredVouchers,
    getClientVouchers,
    calculateServicePayments,
    calculateClientVoucherDetails,
    calculateVoucherRemaining,
  ]);
  ...
  const hasFiltersApplied = () => {
    ...
    return (
      !filters.month.includes(currentMonth) ||
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 p-4">
        ...
        <Card className="w-full dark:bg-slate-950 mb-6">
              ...
              <div className="px-6 pb-6">
                {(() => {
                  const groupedVouchers =
                    groupVouchersByCurrency(filteredVouchers);
                  const currencyOrder = ["USD", "EUR", "TRY"];
                  const currencies = Object.keys(groupedVouchers).sort(
                    (a, b) => {
                  ...
                  return currencies.map((currency, index) => (
                    <div key={currency} className={index > 0 ? "mt-8" : ""}>
                        ...
                        <div className="mb-4">
                          <h4 className="text-md font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-500 dark:text-gray-400">
                              {groupedVouchers[currency].length} voucher
                              {groupedVouchers[currency].length !== 1
                                ? "s"
                                : ""}
                            </span>
                          </h4>
                      ...
                      <div className="hidden lg:block">
                        <CustomScrollbar maxHeight="450px">
                          <CustomTable
                            ...
                            renderRow={(voucher) => {
                              const services =
                                calculateServicePayments(voucher);
                              const remaining = calculateVoucherRemaining(
                                voucher._id,
                                services.total
                              );
                              return (
                      ...
                      <div className="lg:hidden space-y-4">
                        {groupedVouchers[currency].map((voucher) => {
                          const services = calculateServicePayments(voucher);
                          const remaining = calculateVoucherRemaining(
                            voucher._id,
                            services.total
                          ...
                          return (
                            <div
                              key={voucher._id}
                              className="bg-slate-900/60 dark:bg-slate-900 rounded-2xl p-4 border border-slate-800 shadow-inner space-y-3"
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs px-2 py-1 rounded-full bg-slate-800 text-gray-200">
                                  {voucher.currency}
                                </span>
                              </div>
                              <div>
                              ...
                              <div className="grid grid-cols-2 gap-3 text-sm">
                                ...
                                <div className="bg-slate-800/70 rounded-xl p-3">
                                  <p className="text-orange-400 text-xs uppercase tracking-wide">
                                    Flights
                                  </p>
                                  <p className="text-white font-semibold">
                                    {getCurrencySymbol(voucher.currency)}
                                ...
                                <div className="bg-slate-800/70 rounded-xl p-3">
                                  <p className="text-pink-400 text-xs uppercase tracking-wide">
                                    Others
                                  </p>
                                  <p className="text-white font-semibold">
                                    {getCurrencySymbol(voucher.currency)}
          ...
          {getClientVouchers.length > 0 && (
            <div className="p-6 pt-0">
              ...
              {(() => {
                const groupedClientVouchers =
                  groupVouchersByCurrency(getClientVouchers);
                const currencyOrder = ["USD", "EUR", "TRY"];
                const currencies = Object.keys(groupedClientVouchers).sort(
                  (a, b) => {
                    const indexA = currencyOrder.indexOf(a);
                    const indexB = currencyOrder.indexOf(b);
                    if (indexA === -1 && indexB === -1)
                ...
                return currencies.map((currency, index) => (
                  <div key={currency} className={index > 0 ? "mt-8" : ""}>
                    {filters.currency === "ALL" && (
                      <div className="mb-4">
                        <h4 className="text-md font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-500 dark:text-gray-400">
                            {groupedClientVouchers[currency].length} voucher
                            {groupedClientVouchers[currency].length !== 1
                              ? "s"
                              : ""}
                          </span>
                        </h4>
                    ...
                    <div className="hidden lg:block">
                      <CustomScrollbar maxHeight="450px">
                        <CustomTable
                          ...
                          renderRow={(voucher) => {
                            const details =
                              calculateClientVoucherDetails(voucher);
                            return (
                              <>
                                <Table.Cell className="font-medium text-sm text-gray-900 dark:text-white px-4 py-3">
                                  <button
                    ...
                    <div className="lg:hidden space-y-4">
                      {groupedClientVouchers[currency].map((voucher) => {
                        const details = calculateClientVoucherDetails(voucher);
                        return (
                          <div
                            key={voucher._id}
              ...
              <div className="p-6 text-center">
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 max-w-md">
                    This office has no financial activity with the current
                    filters. Try adjusting the date range or currency filter to
                    see more data.
                  </p>
                </div>
              </div>
          ...
          <Card className="w-full dark:bg-slate-950">
            <div className="p-6">
              <PaymentManager
                ...
                originalTotal={
                  isDirectClient
                    ? totals.clientTotalAmount
                    : totals.servicesProvided
                }
                currency={filters.currency}
                filters={filters}
      ...
      <ScrollToTop position="left" className="hidden md:block" />
    </div>
  );
};
