...
const PublicPackagesPage = () => {
  ...
  useEffect(() => {
    ...
    const existingOgLocales = document.querySelectorAll(
  ...
  const fetchRecentPackages = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `/api/packages/recent?limit=4${langParam}`
      );
  ...
  // Fetch packages with server-side pagination and filtering (SINGLE FETCH)
  const fetchPackages = useCallback(async () => {
    try {
      ...
      // Fetch all packages once (from cache after first load)
      const response = await axios.get(
        `/api/packages/featured?limit=100${langParam}`
      );

      if (response.data?.success) {
        const allPackages = response.data.data || [];

        // Store all packages for filter options (only if not already stored)
        if (allPackagesData.length === 0) {
          setAllPackagesData(allPackages);

          // Extract unique countries and cities for filters
          const countries = [
            ...new Set(
              allPackages.flatMap((pkg) => pkg.countries || []).filter(Boolean)
            ),
          ].sort();
          const cities = [
            ...new Set(allPackages.flatMap((pkg) => pkg.cities || [])),
          ].sort();
        ...
        if (countryFilter) {
          filteredPackages = filteredPackages.filter((pkg) =>
            pkg.countries?.includes(countryFilter)
          );
        }
        ...
        if (cityFilter) {
          filteredPackages = filteredPackages.filter((pkg) =>
            pkg.cities?.includes(cityFilter)
          );
        }
        ...
        if (durationFilter) {
          ...
          if (duration === 1) {
            ...
            filteredPackages = filteredPackages.filter(
              (pkg) => pkg.duration > 3 && pkg.duration <= 7
            );
          } else if (duration === 3) {
            filteredPackages = filteredPackages.filter(
              (pkg) => pkg.duration > 7
        ...
        if (targetAudienceFilter) {
          filteredPackages = filteredPackages.filter((pkg) =>
            pkg.targetAudience?.includes(targetAudienceFilter)
          );
        }
  ...
  // Fetch recent packages on mount
  useEffect(() => {
    fetchRecentPackages();
  }, [fetchRecentPackages]);
                        ...
                        <div className="mb-4">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <h3 className="text-base font-bold text-gray-900 dark:text-white">
                              {t("packagesPage.searchAndFilter")}
                            </h3>
                          </div>
                          ...
                          <div className="mb-3">
                            <SearchableSelect
                              value={countryFilter}
                              onChange={(e) =>
                                handleCountryFilter(e.target.value)
                              }
                              ...
                              options={[
                                {
                                  value: "",
                                  label: t("packagesPage.allCountries"),
                                },
                          ...
                          <div className="mb-3">
                            <SearchableSelect
                              ...
                              options={[
                                {
                                  value: "",
                                  label: t("packagesPage.allCities"),
                                },
                                ...availableCities
                                  .filter(
                                    (city) =>
                          ...
                          <div className="mb-3">
                            <SearchableSelect
                              ...
                              onChange={(e) =>
                                handleTargetAudienceFilter(e.target.value)
                              }
                              ...
                              options={[
                                {
                                  value: "",
                                  label: t("packagesPage.allAudiences"),
                                },
                                {
                                  value: "Family",
                                  label: t("packagesPage.family"),
                                },
                                {
                                  value: "Couples",
                                  label: t("packagesPage.couples"),
                                },
                                {
                                  value: "Solo Travelers",
                                  label: t("packagesPage.soloTravelers"),
                                },
                                {
                                  value: "Groups",
                                  label: t("packagesPage.groups"),
                                },
                                {
                                  value: "Business",
                                  label: t("packagesPage.business"),
                                },
                                {
                                  value: "Luxury",
                                  label: t("packagesPage.luxury"),
                                },
                                {
                                  value: "Budget",
                                  label: t("packagesPage.budget"),
                                },
                              ]}
                            />
                          </div>
                          ...
                          <div className="mb-3">
                            <SearchableSelect
                              ...
                              onChange={(e) =>
                                handleDurationFilter(e.target.value)
                              }
                              ...
                              options={[
                                {
                                  value: "",
                                  label: t("packagesPage.anyDuration"),
                                },
                                {
                                  value: "1",
                                  label: t("packagesPage.shortDuration"),
                                },
                                {
                                  value: "2",
                                  label: t("packagesPage.mediumDuration"),
                                },
                                {
                                  value: "3",
                                  label: t("packagesPage.longDuration"),
                                },
                              ]}
                            />
                          </div>
                          ...
                          <div className="text-xs text-gray-600 dark:text-gray-400 mt-3 text-center">
                            {totalPackages > 0 ? (
                              <>
                                {t("packagesPage.showingPage")} {page}{" "}
                                {t("packagesPage.of")} {totalPages} (
                                {totalPackages}{" "}
                                {t("packagesPage.packagesTotal")})
                              </>
                            ) : (
