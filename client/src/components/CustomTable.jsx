...
const CustomTable = ({
  ...
  const customTheme = {
    root: {
      ...
      shadow:
        "absolute bg-white dark:bg-slate-900 w-full h-full top-0 left-0 rounded-lg drop-shadow-md -z-10",
      wrapper: "relative",
    },
    body: {
      base: "group/tbody",
    ...
    row: {
      ...
      striped:
        "odd:bg-white even:bg-gray-50 odd:dark:bg-slate-800 even:dark:bg-slate-900",
    },
  };
  ...
  return (
    <>
      ...
      <div className={`custom-table-wrapper ${className}`}>
        <Table striped={striped} theme={customTheme}>
          {headers.length > 0 && (
