...
export default function EmailSchedulerPanel({ isAdmin, notificationLoading, setNotificationLoading }) {
        ...
        try {
            ...
            const updateData = {
                metadata: {
                    hour: hour,
                    minute: minute,
                    dayOfWeek: editingSchedule.dayOfWeek.length > 0 ? editingSchedule.dayOfWeek : undefined,
            ...
            await axios.put(`/api/scheduler/schedules/${editingSchedule.jobName}`, updateData, {
                headers: { Authorization: `Bearer ${token}` }
            });

            toast.success(`Schedule for ${editingSchedule.jobName} updated successfully!`, {
                duration: 3000,
                style: {
                    background: '#4CAF50',
                    color: '#fff',
    ...
    const handleToggleJob = async (jobName, enabled) => {
        try {
            ...
            await axios.patch(`/api/scheduler/schedules/${jobName}/toggle`, 
                { enabled }, 
                { headers: { Authorization: `Bearer ${token}` } }
            );

            toast.success(`${jobName} ${enabled ? 'enabled' : 'disabled'} successfully!`, {
                duration: 3000,
                style: {
                    background: '#4CAF50',
                    color: '#fff',
    ...
    const handleUpdateTimezone = async () => {
        try {
            ...
            await axios.put('/api/scheduler/timezone', 
    ...
    const handleTriggerSchedulerJob = async (jobName) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`/api/scheduler/trigger/${jobName}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            
            toast.success(response.data.message || `Job '${jobName}' triggered successfully`, {
                duration: 3000,
                style: {
                    background: '#4CAF50',
                    color: '#fff',
            ...
            toast.error(err.response?.data?.message || `Failed to trigger job '${jobName}'`, {
                duration: 3000,
                style: {
                    background: '#f44336',
                    color: '#fff',
    ...
    return (
        <Card className="w-full dark:bg-slate-950 mx-auto max-w-7xl" id="scheduler-panel" role="tabpanel" aria-labelledby="tab-scheduler">
                    ...
                    {schedulesLoading ? (
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {schedules.map((schedule) => {
                                ...
                                return (
                                    <div key={schedule.jobName} className="bg-white/60 dark:bg-slate-800/60 p-3 rounded-lg border border-teal-100 dark:border-teal-800/30">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center space-x-2 flex-shrink-0">
                                                ...
                                                <button
                                                    onClick={() => handleToggleJob(schedule.jobName, !schedule.enabled)}
                                                    disabled={notificationLoading}
            ...
            <CustomModal
                ...
                title={`Edit Schedule: ${editingSchedule && formatJobName(editingSchedule.jobName)}`}
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleSaveSchedule(); }} className="space-y-4">
                        ...
                        {!editingSchedule.intervalSeconds && (
                            <TextInput
                                type="time"
                                label="Schedule Time"
                                value={editingSchedule.timeDisplay?.includes('AM') || editingSchedule.timeDisplay?.includes('PM') 
                                    ? convertTo24Hour(editingSchedule.timeDisplay) 
                                    : `${(editingSchedule.hour || 0).toString().padStart(2, '0')}:${(editingSchedule.minute || 0).toString().padStart(2, '0')}`}
                                onChange={(e) => setEditingSchedule({
                                    ...editingSchedule,
                        ...
                        {editingSchedule.jobName === 'upcoming-events' && (
                            <div>
                                <Label value="Days of Week (for upcoming events)" className="text-gray-700 dark:text-gray-200 font-medium mb-2 block" />
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                    {[
                                        ...
                                        <CustomCheckbox
                                            ...
                                            id={`day-${day.value}`}
                                            label={day.label}
                                            checked={editingSchedule.dayOfWeek?.includes(day.value) || false}
                                            onChange={(checked) => {
                        ...
                        {editingSchedule.jobName === 'monthly-financial' && (
                            <div>
                                <Select
                                    ...
                                    onChange={(value) => setEditingSchedule({
                                        ...editingSchedule,
                        ...
                        <div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            ...
                            />
                            <Label htmlFor="enabled" value="Enable this schedule" className="text-gray-700 dark:text-gray-200" />
                        </div>
            ...
            >
                <form onSubmit={(e) => { e.preventDefault(); handleUpdateTimezone(); }} className="space-y-4">
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-700">
                        <p className="text-sm text-blue-800 dark:text-blue-200">
                            📍 Changing the timezone will restart all scheduled jobs and update their execution times.
                            This affects all email notifications system-wide.
                        </p>
                    </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4 border-t dark:border-gray-600">
                        ...
                        <CustomButton
                            type="submit"
                            variant="orange"
                            disabled={notificationLoading || selectedTimezone === currentTimezone}
                            loading={notificationLoading}
                        >
