...
export default function Dashboard() {
  ...
  const [attendanceTooltip, setAttendanceTooltip] = useState({
    visible: false,
    x: 0,
    y: 0,
  });
  const [verifiedEmailsTooltip, setVerifiedEmailsTooltip] = useState({
    visible: false,
    x: 0,
    y: 0,
  });
  ...
  const getTrendIcon = (trend) => {
    if (trend > 0)
      return (
        <FaArrowUp className="w-3 h-3 text-green-500 dark:text-green-400" />
      );
    if (trend < 0)
      return <FaArrowDown className="w-3 h-3 text-red-500 dark:text-red-400" />;
  ...
  const availableCurrencies = React.useMemo(() => {
    if (!analytics?.charts?.monthlyProfitChart)
    ...
    analytics.charts.monthlyProfitChart.forEach((monthData) => {
      if (monthData.currencies) {
        Object.keys(monthData.currencies).forEach((currency) => {
          ...
          if (
            currencyData &&
            (currencyData.clientRevenue > 0 || currencyData.voucherCount > 0)
          ) {
  ...
  React.useEffect(() => {
    if (
      ...
      !availableCurrencies.find((c) => c.value === selectedCurrency)
    ) {
  ...
  const getRevenueGrowth = () => {
    ...
    const currentRevenue =
      currentMonthData?.currencies?.[selectedCurrency]?.clientRevenue || 0;
    const previousRevenue =
      previousMonthData?.currencies?.[selectedCurrency]?.clientRevenue || 0;
  ...
  const getProfitGrowth = () => {
    ...
    const currentProfit =
      currentMonthData?.currencies?.[selectedCurrency]?.profit || 0;
    const previousProfit =
      previousMonthData?.currencies?.[selectedCurrency]?.profit || 0;
    ...
    <div className="bg-gray-50 dark:bg-slate-950 min-h-screen">
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
                  ...
                  <div className="flex items-center p-2 sm:p-4 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-800/30 rounded-xl border border-red-200 dark:border-red-700 hover:shadow-lg transition-all duration-200">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-base sm:text-2xl font-bold text-red-900 dark:text-red-100 truncate">
                        {getCurrencySymbol(selectedCurrency)}
                        {currentMonthData.costs.toLocaleString()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        <span className="hidden sm:inline">
                          Profit progression for {new Date().getFullYear()}
                        </span>
              ...
              <div className="p-6 space-y-4 flex-1">
                  ...
                  <div className="flex items-center justify-between p-1.5 sm:p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-gray-900 dark:text-white">
                          {analytics?.inventory?.airports || 0}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3 mt-6">
                  ...
                  <div className="flex items-center justify-between p-1.5 sm:p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {analytics?.attendance?.monthlyStats
                        ?.attendancePercentage !== undefined
                        ? `${analytics.attendance.monthlyStats.attendancePercentage}%`
                        : "N/A"}
                    </span>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-slate-950/50 rounded-2xl shadow-xl border-0 overflow-hidden backdrop-blur-sm min-h-[400px] flex flex-col">
            ...
            {/* Content */}
            <div className="p-6 space-y-4 flex-1">
              {!analytics?.recentActivity?.vouchers ||
              analytics.recentActivity.vouchers.length === 0 ? (
                ...
                <CustomScrollbar maxHeight="350px">
                  <div className="space-y-3">
                    {analytics.recentActivity.vouchers.map((voucher) => (
                      <div
                        key={voucher._id}
                        className="flex items-center justify-between p-1.5 sm:p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                      >
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500 dark:text-gray-400">
                              ...
                              • {formatTimeAgo(voucher.createdAt)}
                              {voucher.createdBy && (
                                <>
                                  {" • Created by "}
                                  <Link
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 h-full">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <p className="text-xs sm:text-sm font-medium text-yellow-700 dark:text-yellow-300 text-left">
                      Active (Await)
                    </p>
          ...
          <div className="bg-white dark:bg-slate-950/50 rounded-2xl shadow-xl border-0 overflow-hidden backdrop-blur-sm min-h-[400px] flex flex-col">
            ...
            <div className="flex-1 p-6">
              <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
                ...
                <div className="h-64">
                  {analytics?.charts?.monthlyTrends &&
                  analytics.charts.monthlyTrends.length > 0 ? (
                    <VoucherTrendChart
          ...
          <div className="bg-white dark:bg-slate-950/50 rounded-2xl shadow-xl border-0 overflow-hidden backdrop-blur-sm min-h-[400px] flex flex-col">
            ...
            <div className="flex-1 p-6">
              <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
                ...
                <div className="h-64">
                  {analytics?.charts?.monthlyArrivalTrends &&
                  analytics.charts.monthlyArrivalTrends.length > 0 ? (
                    <MonthlyArrivalChart
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {analytics.recentActivity.users
                      ...
                      .map((user) => {
                        ...
                        const getUserRole = (user) => {
                          if (user.isAdmin)
                            return {
                              ...
                              mobileRole: "Admin",
                              color:
                                "bg-blue-800 text-white border border-blue-800 shadow-md",
                            };
                        ...
                        const hasSalary =
                          user.salaryAmount && user.salaryCurrency;

                        return (
                          <div
                            key={user._id}
                            className="flex items-center justify-between p-3 sm:p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors"
                          >
                            <div className="flex items-center gap-2 sm:gap-4 flex-1">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1 sm:gap-2 mb-1">
                                  ...
                                  <span
                                    className={`inline-flex items-center justify-center rounded-lg text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.5 font-semibold transition-all duration-200 hover:scale-105 hover:shadow-lg w-12 sm:w-16 min-w-12 sm:min-w-16 ${roleInfo.color}`}
                                  >
                                    <span className="sm:hidden">
                                      {roleInfo.mobileRole}
                                    </span>
                              ...
                              {hasSalary ? (
                                <div>
                                  <p className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white">
                                    {getCurrencySymbol(user.salaryCurrency)}
      ...
      {attendanceTooltip.visible && (
        <div
          ...
          style={{
            left: Math.min(
              Math.max(attendanceTooltip.x, 5),
        ...
        >
          ...
          <div className="opacity-75">
            {analytics?.attendance?.today?.total || 0} users have attended today
          </div>
          <div className="opacity-60">
            Checked in: {analytics?.attendance?.today?.checkedIn || 0} | Checked
            out: {analytics?.attendance?.today?.checkedOut || 0}
          </div>
        </div>
      )}
      ...
      {verifiedEmailsTooltip.visible && (
        <div
          ...
          style={{
            left: Math.min(
              Math.max(verifiedEmailsTooltip.x, 5),
              window.innerWidth - 200
            ),
        ...
        >
          ...
          <div className="opacity-75">
            {analytics?.users?.verifiedEmails || 0} users have verified their
            email addresses
          </div>
          <div className="opacity-60">
            Verification rate:{" "}
            {analytics?.users?.approved
              ? Math.round(
                  ((analytics?.users?.verifiedEmails || 0) /
                    analytics.users.approved) *
                    100
