...
import {
  ...
  getUserSalary,
  updateUserSalary,
  getUserSalaryBaseEntries,
  editMonthBonus,
  saveMonthlyBaseSalary,
  editMonthSalary,
  deleteSalaryEntry,
  deleteBonusEntry,
...
export default function AdminPanel() {
  ...
  const isNotificationsOnlyRoute =
    window.location.pathname === "/notifications/manage";
    ...
    if (typeof window !== "undefined") {
      ...
      const availableTabs = isAdmin
        ...
        : [
            ...
            "users",
            "notifications",
          ]; // Accountants can access users tab but not requests, scheduler is admin-only
      if (availableTabs.includes(tabParam)) {
        return tabParam;
      }
    }
    return isPublisher ? "blog" : isContentManager ? "hotels" : "analytics";
  };
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (availableTabs.includes(tabParam)) {
        ...
        setActiveTab(
          isPublisher ? "blog" : isContentManager ? "hotels" : "analytics"
        );
      }
    };
  ...
  const handleTabChange = (tabName) => {
    ...
    if (!isAdmin && tabName === "requests") {
      console.warn(
        "Access denied: Only administrators can access user requests tab"
      );
      return;
    }
    if (
      isPublisher &&
      !["hotels", "tours", "packages", "blog", "ui-management"].includes(
        tabName
      )
    ) {
      console.warn(
        "Access denied: Publishers can only access content publishing tabs"
      );
      return;
    }
    if (
      ...
      console.warn(
        "Access denied: Content managers can only access content management tabs"
      );
      return;
    }
  ...
  const [selectedOfficeForVouchers, setSelectedOfficeForVouchers] =
    useState("");
  ...
  const [updateFromNextCycleAdmin, setUpdateFromNextCycleAdmin] =
    useState(false);
  ...
  const handleMonthSelection = (monthKey) => {
    ...
    if (monthKey) {
      const [year, month] = monthKey.split("-").map(Number);
      const entry = salaryBaseEntries.find(
  ...
  const handleBonusMonthSelection = (monthKey) => {
    ...
    if (monthKey) {
      ...
      const salaryEntry = (salaryBaseEntries || []).find(
        (entry) => entry.year === year && entry.month === month
      );
      const correctCurrency = salaryEntry
        ? salaryEntry.currency
        : selectedUserData?.salaryCurrency || "USD";
      ...
      setBonusForm({
        amount: "",
        currency: selectedUserData?.salaryCurrency || "USD",
        note: "",
      });
    }
  };
  ...
  useEffect(() => {
    ...
    salaryBaseEntries,
    selectedBonusMonthToEdit,
    bonuses,
    selectedUserData?.salaryCurrency,
  ]);
  ...
  const handleLoadUserBonuses = async (userId) => {
    try {
      ...
      if (bonusEntries.length > 0) {
        ...
        const currentMonthBonus = bonusEntries.find(
  ...
  const handleLoadUserSalaryBaseEntries = async (
    userId,
  ...
  const handleSaveAmount = async (entry) => {
    ...
    try {
      ...
      const userResponse = await axios.get(
        `/api/profile/${selectedUserForSalary}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  const openSalaryDeleteConfirmation = (entry) => {
    const monthName = new Date(entry.year, entry.month, 1).toLocaleString(
      undefined,
  ...
  const loadAvailableSalaryOptions = useCallback(async () => {
    try {
      ...
      const filteredUsers =
        isAccountant && !isAdmin ? users.filter((u) => !u.isAdmin) : users;
      ...
      const userPromises = filteredUsers.map(async (user) => {
        try {
          ...
          console.error(
            `Error loading data for user ${user.username}:`,
            userError
          );
          return { baseEntries: [], bonuses: [] };
        }
      ...
      const userPromises = filteredUsers.map(async (user) => {
        try {
          ...
          if (salaryCardsMonth !== "all") {
            ...
            const monthEntry = baseEntries.find(
              (entry) =>
                entry.year === salaryCardsYear &&
                entry.month === parseInt(salaryCardsMonth)
            );
            if (monthEntry) {
          ...
          const filteredBonuses = bonuses.filter((bonus) => {
            ...
            if (
              ...
              bonus.month !== parseInt(salaryCardsMonth)
            )
              return false;
            return true;
          });
          ...
          const totalBonus = filteredBonuses.reduce(
  ...
  const getScheduledRaiseInfoAdmin = () => {
    if (
      !selectedUserData?.salaryBaseEntries ||
      selectedUserData.salaryBaseEntries.length === 0
    )
      return null;
    ...
    const currentMonthEntry = selectedUserData.salaryBaseEntries.find(
      (e) => e.year === now.getFullYear() && e.month === now.getMonth()
    );
    const currentSalary =
      currentMonthEntry?.amount ?? selectedUserData.salaryAmount ?? 0;

    // Next month's salary
    const nextMonthEntry = selectedUserData.salaryBaseEntries.find(
      (e) => e.year === nextYear && e.month === nextMonthIndex
    );
    ...
    if (salaryDifference !== 0) {
      const nextSalaryDate = selectedUserData.salaryDayOfMonth
        ? getNextSalaryDateAdmin(selectedUserData.salaryDayOfMonth)
        : null;
      return {
        amount: salaryDifference,
        currency:
          nextMonthEntry.currency || selectedUserData.salaryCurrency || "USD",
        newTotal: nextSalary,
        date: nextSalaryDate,
  ...
  const handleGrantBonus = async () => {
    ...
    try {
      ...
      if (selectedBonusMonthToEdit) {
        const [year, month] = selectedBonusMonthToEdit.split("-").map(Number);
        const existingBonus = bonuses.find(
        ...
        if (existingBonus) {
          ...
          await saveMonthlyBonus(selected._id, {
            year,
            month,
            amount: amountNum,
            note: bonusForm.note,
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      if (
        currentUserId &&
        !approvedUsers.some((u) => u._id === currentUserId)
      ) {
      ...
      if (currentUserId) {
        ...
        if (
          meIndex !== -1 &&
          typeof approvedUsers[meIndex].salaryAmount === "undefined"
        ) {
          try {
  ...
  const fetchFinancialData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get("/api/vouchers/financials", {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      filteredVouchers.forEach((voucher) => {
        ...
        const monthYear = `${voucherDate.getFullYear()}-${String(
      ...
      const uniqueOffices = [
        ...new Set(financialArray.map((item) => item.officeName)),
      ];

      // Also get unique client offices for client payments
      const uniqueClientOffices = [
        ...new Set(
          filteredVouchers
            .map((voucher) => {
      ...
      const allUniqueOffices = [
        ...new Set([...uniqueOffices, ...uniqueClientOffices]),
      ];
  ...
  useEffect(() => {
    // Mark as loaded immediately - fetch data only when tabs need it
    if (!dataLoaded) {
      setDataLoaded(true);
    }
  }, [dataLoaded]);

  // Fetch hotels and offices when needed
  useEffect(() => {
    const needsHotelsOffices = [
      "hotels",
      "offices",
      "office-vouchers",
      "financials",
    ...
    if (
      ...
      const fetchHotelsOffices = async () => {
        try {
          ...
          console.error("Failed to fetch hotels/offices:", err);
        }
      };
      fetchHotelsOffices();
    }
  }, [activeTab, dataLoaded, hotels.length, offices.length]);
  ...
  }, [activeTab, dataLoaded, fetchUsers, users.length]);
    ...
    if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
      ...
      const tabs = isAdmin
        ...
        : [
            ...
            "office-vouchers",
            "financials",
          ]; // Accountants can access financials but not users/requests
  ...
  const handleGenerateArrivalReminders = async () => {
    ...
    try {
      ...
      toast.success(
        result.message || "Arrival reminders generated successfully",
        {
          duration: 3000,
          style: {
            background: "#4CAF50",
      ...
      toast.error(
        err.response?.data?.message || "Failed to generate arrival reminders",
        {
          duration: 3000,
          style: {
            background: "#f44336",
  ...
  const handleGenerateDepartureReminders = async () => {
    ...
    try {
      ...
      toast.success(
        result.message || "Departure reminders generated successfully",
        {
          duration: 3000,
          style: {
            background: "#4CAF50",
      ...
      toast.error(
        err.response?.data?.message || "Failed to generate departure reminders",
        {
          duration: 3000,
          style: {
            background: "#f44336",
  ...
  const handleGenerateDailySummary = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "/api/notifications/generate-daily-summary",
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      toast.success(
        response.data.message || "Daily summary generated successfully",
        {
          duration: 3000,
          style: {
            background: "#4CAF50",
      ...
      toast.error(
        err.response?.data?.message || "Failed to generate daily summary",
        {
          duration: 3000,
          style: {
            background: "#f44336",
  ...
  const handleCleanupExpiredNotifications = async () => {
    ...
    try {
      ...
      toast.success(
        result.message || "Expired notifications cleaned up successfully",
        {
          duration: 3000,
          style: {
            background: "#4CAF50",
  ...
  const fetchOfficeVouchers = async (officeName) => {
    ...
    try {
      ...
      const filteredVouchers = response.data.filter(
        (voucher) => voucher.officeName === officeName
      );
  ...
  const handleOfficeSelection = async (eventOrOfficeName) => {
    ...
    const officeName =
      typeof eventOrOfficeName === "string"
        ? eventOrOfficeName
  ...
  const handlePaymentDateUpdate = async (voucherId, paymentDate) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `/api/vouchers/${voucherId}/payment-date`,
        { paymentDate },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setOfficeVouchers((prevVouchers) =>
        prevVouchers.map((voucher) =>
          voucher._id === voucherId ? { ...voucher, paymentDate } : voucher
        )
      );
  ...
  const filteredFinancialData = financialData.filter((item) => {
    if (financialFilters.year && item.year.toString() !== financialFilters.year)
      return false;

    // Only apply month filter if a specific month is selected (not "All Months")
    if (
      financialFilters.month &&
      typeof financialFilters.month === "string" &&
      financialFilters.month.trim() !== ""
    ) {
      const expectedMonth = `${
        financialFilters.year
      }-${financialFilters.month.padStart(2, "0")}`;
  ...
  const financialTotals = filteredFinancialData.reduce(
    (acc, item) => {
  ...
  const totalClientRevenue = useMemo(() => {
    ...
    const filteredVouchers = allVouchers.filter((voucher) => {
      ...
      if (
        financialFilters.year &&
        voucherYear.toString() !== financialFilters.year
      )
        return false;
    ...
    return filteredVouchers.reduce(
      (total, voucher) => total + (voucher.totalAmount || 0),
      0
    );
  }, [
    allVouchers,
  ...
  const totalSupplierRevenue = useMemo(() => {
    ...
    const filteredSupplierData = financialData.filter((item) => {
      if (
        financialFilters.year &&
        item.year.toString() !== financialFilters.year
      )
        return false;
    ...
    return filteredSupplierData.reduce(
  ...
  const clientOfficeData = useMemo(() => {
    ...
    filteredVouchers.forEach((voucher) => {
      ...
      const monthName = voucherDate.toLocaleString("default", {
        month: "long",
      });
    ...
    return Array.from(clientOfficesMap.values())
      .filter((office) => {
        ...
        if (financialSearchQuery.trim()) {
          const searchLower = financialSearchQuery.toLowerCase();
          if (!office.officeName.toLowerCase().includes(searchLower))
            return false;
        }

        // Apply client type filter
        if (clientTypeFilter === "office" && office.isDirectClient)
          return false;
        if (clientTypeFilter === "direct" && !office.isDirectClient)
          return false;
  ...
  const downloadFinancialSummaryPDF = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "/api/notifications/download-financial-summary-pdf",
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { year, month },
          responseType: "blob",
  ...
  const handleDebtSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(
        `Debt ${editingDebt ? "updated" : "created"} successfully!`,
        {
          duration: 3000,
          style: {
            background: "#4CAF50",
  ...
  const hasFinancialFiltersApplied = () => {
    ...
    return (
      financialFilters.month !== currentMonth ||
    ...
    <div className="w-full flex flex-col items-center">
      ...
      {!isNotificationsOnlyRoute && (
        <div className="lg:hidden w-full fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-gray-700 shadow-lg">
          <div className="overflow-x-auto scrollbar-hide">
            <div
              className="flex space-x-1 px-4 py-2 min-w-max"
              role="tablist"
              aria-label="Admin Sections"
            >
              {!isContentManager && !isPublisher && (
                <button
                  id="tab-analytics-mobile"
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col md:flex-row w-full">
          ...
          <div
            className={`flex-1 ${!isNotificationsOnlyRoute ? "lg:ml-16" : ""}`}
          >
            {/* Tab panels */}
            {activeTab === "analytics" && !isContentManager && !isPublisher && (
              <div
                id="analytics-panel"
                role="tabpanel"
            ...
            {activeTab === "office-vouchers" && (isAdmin || isAccountant) && (
              <Card
                ...
                id="office-vouchers-panel"
                role="tabpanel"
                aria-labelledby="tab-office-vouchers"
              >
                    ...
                    {officeVouchersLoading ? (
                            ...
                            <CustomTable
                              ...
                              renderRow={(voucher) => (
                                <>
                                  ...
                                  <Table.Cell className="text-sm text-gray-900 dark:text-white px-4 py-3">
                                    {voucher.advancedPayment &&
                                    voucher.advancedAmount
                                      ? `${getCurrencySymbol(
                                          voucher.currency
                                        )}${voucher.advancedAmount}`
                                      : "N/A"}
                                  ...
                                  <Table.Cell className="px-4 py-3">
                                    {(() => {
                                      ...
                                      const hasApprovedPayments =
                                        voucher.officePayments &&
                                        voucher.officePayments.some(
                                          (payment) =>
                                      ...
                                      const hasPayments =
                                        voucher.payments &&
                                        Object.values(voucher.payments).some(
                                          (payment) =>
                                            payment.officeName ===
                                              selectedOfficeForVouchers &&
                                            payment.price > 0
                                ...
                                {Object.entries(totalsByCurrency).map(
                                  ([currency, amounts]) => {
                                    ...
                                    return (
                                      <div
                                        key={currency}
                                        className={`p-4 rounded-lg border ${getCardColor(
                                          currency
                                        )}`}
                                      >
                                        <h4
                                          className={`text-lg font-semibold mb-2 ${getHeaderColor(
                                            currency
                                          )}`}
                                        >
                                          {currency} Total
                                        ...
                                        <div className="space-y-1">
                                          <div className="flex justify-between text-sm">
                                            <span
                                              className={getTextColor(currency)}
                                            >
                                              Total Amount:
                                            </span>
                                            <span
                                              className={`font-medium ${getBoldTextColor(
                                                currency
                                              )}`}
                                            >
                                          ...
                                          <div className="flex justify-between text-sm">
                                            ...
                                            >
                                              {getCurrencySymbol(currency)}
                                              {amounts.totalAdvanced.toFixed(2)}
                                            </span>
                                          </div>
                                        </div>
                                      </div>
                      ...
                      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                        ...
                        <p className="text-lg font-medium text-gray-700 dark:text-gray-300">
                          No vouchers found for {selectedOfficeForVouchers}
                        </p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          This office doesn't have any vouchers yet.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {!selectedOfficeForVouchers && (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <p className="text-lg font-medium">
                      Select an office to view vouchers
                    </p>
                    <p className="text-sm">
                      Choose an office from the dropdown above to see all
                      vouchers assigned to that office.
                    </p>
                  </div>
                )}
              </Card>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 md:mb-6 gap-4">
                  <h2 className="text-xl md:text-2xl font-bold dark:text-white flex items-center">
                    <svg
                      ...
                      className="h-8 w-8 mr-3 text-green-600 dark:text-green-400"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                  ...
                  {financialFilters.viewType === "providers" ? (
                    <>
                      ...
                      <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-blue-200 dark:border-blue-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium text-blue-600 dark:text-blue-400 leading-tight">
                              Suppliers Cost
                            </p>
                            <p className="text-sm md:text-2xl font-bold text-blue-900 dark:text-blue-100 leading-tight mt-1">
                              ...
                              {financialTotals.total.toLocaleString()}
                            </p>
                          </div>
                      ...
                      <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-green-200 dark:border-green-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium text-green-600 dark:text-green-400 leading-tight">
                              Active Suppliers
                            </p>
                      ...
                      <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-purple-200 dark:border-purple-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium text-purple-600 dark:text-purple-400 leading-tight">
                              Total Vouchers
                            </p>
                      ...
                      {/* Profit */}
                      <div
                        className={`bg-gradient-to-br p-3 md:p-6 rounded-lg md:rounded-xl border ${
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            ...
                            <p
                              className={`text-sm md:text-2xl font-bold leading-tight mt-1 ${
                                totalClientRevenue - totalSupplierRevenue >= 0
                            ...
                            >
                              {getCurrencySymbol(financialFilters.currency)}
                              {(
                                totalClientRevenue - totalSupplierRevenue
                              ).toLocaleString()}
                            </p>
                          </div>
                          <div
                            className={`w-8 h-8 md:w-12 md:h-12 rounded-md md:rounded-md flex items-center justify-center flex-shrink-0 ${
                              totalClientRevenue - totalSupplierRevenue >= 0
                    ...
                    <>
                      ...
                      <div className="bg-gradient-to-br from-cyan-50 to-cyan-100 dark:from-cyan-900/20 dark:to-cyan-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-cyan-200 dark:border-cyan-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium leading-tight text-cyan-600 dark:text-cyan-400">
                              Total Revenue
                            </p>
                            <p className="text-sm md:text-2xl font-bold leading-tight mt-1 text-cyan-900 dark:text-cyan-100">
                              ...
                              {clientOfficeData
                                .reduce(
                                  (sum, office) => sum + office.totalAmount,
                                  0
                                )
                                .toLocaleString()}
                            </p>
                      ...
                      <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-indigo-200 dark:border-indigo-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium leading-tight text-indigo-600 dark:text-indigo-400">
                              Clients
                            </p>
                      ...
                      <div className="bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-900/20 dark:to-rose-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-rose-200 dark:border-rose-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium leading-tight text-rose-600 dark:text-rose-400">
                              Direct Clients
                            </p>
                            <p className="text-sm md:text-2xl font-bold leading-tight mt-1 text-rose-900 dark:text-rose-100">
                              {
                                [
                                  ...new Set(
                                    clientOfficeData
                                      .filter((office) => office.isDirectClient)
                      ...
                      <div className="bg-gradient-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 p-3 md:p-6 rounded-lg md:rounded-xl border border-teal-200 dark:border-teal-700">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0 flex-1 mr-2">
                            <p className="text-xs md:text-sm font-medium leading-tight text-teal-600 dark:text-teal-400">
                              Total Vouchers
                            </p>
                            <p className="text-sm md:text-2xl font-bold leading-tight mt-1 text-teal-900 dark:text-teal-100">
                              {clientOfficeData
                                .reduce(
                                  (sum, office) => sum + office.voucherCount,
                                  0
                                )
                                .toLocaleString()}
                            </p>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div
                            className={`w-8 h-8 md:w-12 md:h-12 rounded-md md:rounded-lg flex items-center justify-center flex-shrink-0 ${
                              totalClientRevenue - totalSupplierRevenue >= 0
                                ? "bg-green-500 dark:bg-green-600"
                                : "bg-red-500 dark:bg-red-600"
                            }`}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                        ...
                        <CustomSelect
                          ...
                          options={(() => {
                            ...
                            if (selectedYear === currentYear) {
                              allMonths = [
                                ...new Set([currentMonth, ...dataMonths]),
                              ];
                            }
                      ...
                      <div className="md:hidden">
                        <CustomSelect
                          ...
                          options={(() => {
                            ...
                            const dataYears = [
                              ...new Set(
                                financialData.map((item) => item.year)
                              ),
                            ];
                    ...
                    <div className="flex items-end col-span-2 md:col-span-1">
                      <CustomButton
                        ...
                        title={
                          hasFinancialFiltersApplied()
                            ? "Clear all financial filters"
                            : "No filters to clear"
                        }
                        icon={HiX}
                      >
                  ...
                  <div className="flex gap-4 items-center">
                    <div className="w-80">
                      <Search
                        ...
                        onChange={(e) =>
                          setFinancialSearchQuery(e.target.value)
                        }
                      />
                    </div>
            ...
            {activeTab === "debts" && (isAdmin || isAccountant) && (
              ...
              >
                <h2 className="text-xl md:text-2xl font-bold mb-1 dark:text-white mx-auto">
                  Debt Management
                </h2>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                          <SearchableSelect
                            ...
                            onChange={(e) =>
                              handleDebtFilterChange("office", e.target.value)
                            }
                            options={[
                        ...
                        <div>
                          <SearchableSelect
                            ...
                            onChange={(e) =>
                              handleDebtFilterChange("status", e.target.value)
                            }
                            options={[
                        ...
                        <div>
                          <SearchableSelect
                            ...
                            onChange={(e) =>
                              handleDebtFilterChange("type", e.target.value)
                            }
                            options={[
                              { value: "", label: "All Types" },
                              {
                                value: "OWED_TO_OFFICE",
                                label: "We Owe Them",
                              },
                              {
                                value: "OWED_FROM_OFFICE",
                                label: "They Owe Us",
                              },
                            ]}
                        ...
                        <div className="flex items-end">
                          <CustomButton
                            ...
                            title={
                              hasDebtFiltersApplied()
                                ? "Clear all debt filters"
                                : "No filters to clear"
                            }
                            icon={HiX}
                          >
                        ...
                        <div className="sm:hidden space-y-5">
                          {debts.filter((debt) => debt.status === "OPEN")
                            ...
                            <div className="text-center py-12">
                              ...
                              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                                Create debt records to track financial
                                obligations with offices.
                              </p>
                            </div>
                          ) : (
                            debts
                  ...
                  <div>
                                  ...
                                  {debt.amountInUSD &&
                                    debt.currency !== "USD" && (
                                      <span className="text-xs text-gray-500 dark:text-gray-400">
                                        ≈ ${debt.amountInUSD.toLocaleString()}
                                      </span>
                                    )}
                                </div>
                              </Table.Cell>
                    ...
                    <div className="md:hidden">
                      {debtLoading ? (
                        ...
                        <div className="sm:hidden space-y-5">
                          {debts.filter((debt) => debt.status === "CLOSED")
                            .length === 0 ? (
                            <div className="text-center py-12">
                              <svg
                                className="mx-auto h-12 w-12 text-gray-400 mb-4"
                              ...
                              <p className="text-gray-500 dark:text-gray-400 font-medium">
                                No archived debts found
                              </p>
                              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                                Closed debt records will appear here.
                              </p>
                            </div>
                          ) : (
                            debts
            ...
            {activeTab === "salaries" && (isAdmin || isAccountant) && (
              <Card
                ...
                id="salaries-panel"
                role="tabpanel"
                aria-labelledby="tab-salaries"
              >
                <h2 className="text-2xl font-bold mb-4 dark:text-white mx-auto">
                  Salaries & Bonuses
                </h2>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full sm:w-auto">
                        <div>
                          ...
                          <CustomSelect
                            ...
                            onChange={(value) =>
                              setSalaryCardsYear(parseInt(value))
                            }
                            options={
                              availableSalaryYears.length > 0
                          ...
                          <CustomSelect
                            ...
                            options={[
                              ...
                              ...availableSalaryMonths.map((month) => ({
                                value: month.toString(),
                                label: new Date(2024, month).toLocaleDateString(
                                  "en-US",
                                  {
                                    month: "long",
                                  }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
                          {allUsersSalaryData.map((userData, index) => {
                            const totalCompensation =
                              userData.salary + userData.totalBonus;
                            const bonusPercentage =
                              userData.salary > 0
                                ? (userData.totalBonus / userData.salary) * 100
                                : 0;
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-3 overflow-hidden">
                                      <div
                                        ...
                                        style={{
                                          ...
                                          animation: `progressScale 0.6s ease-out ${
                                            index * 150
                                          }ms forwards`,
                                        }}
                                      ></div>
                                  ...
                                  {userData.totalBonus > 0 && (
                                    <div className="text-xs text-center text-gray-500 dark:text-gray-400">
                                      Bonus: {bonusPercentage.toFixed(1)}% of
                                      base salary
                                    </div>
                                  )}
                                </div>
                    ...
                    {!salaryCardsLoading && allUsersSalaryData.length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                          Salary information will appear here once configured
                          for employees.
                        </p>
                      </div>
                    )}
                  ...
                  <div>
                        ...
                        <SearchableSelect
                          ...
                          onChange={async (val) => {
                            ...
                            if (isAccountant && !isAdmin && selected?.isAdmin) {
                              toast.error(
                                "Accountants cannot manage admin salaries",
                                {
                                  duration: 3000,
                                  style: {
                                    background: "#f44336",
                    ...
                    {selectedUserForSalary && salaryInnerTab === "salary" && (
                      <div>
                        ...
                        {(isAdmin || isAccountant) &&
                          salaryBaseEntries &&
                          salaryBaseEntries.length > 0 && (
                            <div className="mb-6">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                  <Label
                                    value="Select Month to Edit"
                                    className="mb-2"
                                  />
                                  <SearchableSelect
                        ...
                        <div className="mt-4">
                                    ...
                                    <div className="flex-1">
                                      <div className="text-sm font-medium text-gray-800 dark:text-gray-200">
                                        ...
                                        {raiseInfo.isIncrease
                                          ...
                                          : "decrease"}
                                        : {raiseInfo.isIncrease ? "+" : ""}
                                        {raiseInfo.amount.toFixed(2)}{" "}
                                        {raiseInfo.currency} effective{" "}
                                        {raiseInfo.date}
                                      </div>
                                      <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                        Current:{" "}
                                        {(
                                          raiseInfo.newTotal - raiseInfo.amount
                                        ).toFixed(2)}{" "}
                                        {raiseInfo.currency} → New:{" "}
                                        <span
                                          className={`${
                                            raiseInfo.isIncrease
                                              ? "text-green-600 dark:text-green-400"
                                    ...
                                    <CustomButton
                                      onClick={async () => {
                                        ...
                                        try {
                                          ...
                                          await axios.delete(
                                            `/api/profile/${selectedUserForSalary}/salary/base`,
                                            {
                                              data: {
                                                year: nextYear,
                                          ...
                                          await handleLoadUserSalaryBaseEntries(
                                            selectedUserForSalary,
                                            false
                                          );

                                          toast.success(
                                            "Scheduled salary change removed",
                                            {
                                              duration: 3000,
                                              style: {
                                                background: "#4CAF50",
                                          ...
                                          console.error(
                                            "Error removing scheduled salary:",
                                            error
                                          );
                                          toast.error(
                                            "Failed to remove scheduled salary change",
                                            {
                                              duration: 3000,
                                              style: {
                                                background: "#f44336",
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                              ...
                              {updateFromNextCycleAdmin &&
                                selectedUserData &&
                                salaryForm.salaryAmount && (
                                  <div className="mt-1 text-xs text-gray-600 dark:text-gray-400">
                                    ...
                                    {(() => {
                                      ...
                                      const currentSalary = currentSalaryEntry
                                        ? currentSalaryEntry.amount.toFixed(2)
                                        : (
                                            selectedUserData.salaryAmount || 0
                                          ).toFixed(2);
                                      const currency = currentSalaryEntry
                                        ? currentSalaryEntry.currency
                                        : selectedUserData.salaryCurrency ||
                                          "USD";
                                      const newSalary = (
                                        parseFloat(salaryForm.salaryAmount) || 0
                                      ).toFixed(2);
                              ...
                              {isCurrentMonth(selectedMonthToEdit) && (
                                <div className="mt-3">
                                  <div className="flex items-center space-x-2">
                                    ...
                                    />
                                    <label
                                      htmlFor="updateFromNextCycleAdmin"
                                      className="text-xs font-medium text-gray-700 dark:text-gray-200"
                                    >
                                      Update from next cycle{" "}
                                      {salaryForm.salaryDayOfMonth
                                        ? `(${(() => {
                                            const day = parseInt(
                                              salaryForm.salaryDayOfMonth,
                                              10
                              ...
                              <div className="h-[42px] flex items-center px-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-300">
                                {salaryForm.salaryDayOfMonth
                                  ? (() => {
                                      ...
                                      const y =
                                        now.getFullYear() + Math.floor(nm / 12);
                                      const m = nm % 12;
                                      const dim = new Date(
                                        y,
                                        m + 1,
                          ...
                          <div className="flex justify-end mt-4">
                            <CustomButton
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  if (updateFromNextCycleAdmin) {
                                    const payload = {
                                      salaryAmount:
                                        parseFloat(salaryForm.salaryAmount) ||
                                        0,
                                  ...
                                  await handleLoadUserSalaryBaseEntries(
                                    selectedUserForSalary,
                                    false
                                  ); // Don't auto-select month after save

                                  // Refresh salary form data to show updated values
                                  await handleLoadUserSalary(
                                    selectedUserForSalary
                                  );
                              ...
                              disabled={
                                !selectedUserForSalary || salaryTabLoading
                              }
                            >
                              {selectedMonthToEdit
                                ? `Save ${
                                    getMonthOptions().find(
                                      (opt) => opt.value === selectedMonthToEdit
                                    )?.label || "Month"
                                  } Salary`
                                : "Save Salary"}
                            </CustomButton>
                                  ...
                                  <Table.Cell className="px-4 py-3 text-center align-middle">
                                    <div className="flex justify-center items-center h-full">
                                      <CustomButton
                                        ...
                                        size="sm"
                                        onClick={() =>
                                          openSalaryDeleteConfirmation(entry)
                                        }
                          ...
                          >
                            {selectedBonusMonthToEdit
                              ? `Save ${
                                  getMonthOptions().find(
                                    (opt) =>
                                      opt.value === selectedBonusMonthToEdit
                                  )?.label || "Month"
                                } Bonus`
                              : "Save Bonus for Previous Cycle"}
                          </CustomButton>
                        </div>
                          ...
                          <CustomTable
                            ...
                            renderRow={(bonus) => (
                              <>
                                ...
                                <Table.Cell className="text-sm text-gray-800 dark:text-gray-100 px-4 py-3">
                                  {editingBonusAmount ===
                                  `${bonus.year}-${bonus.month}` ? (
                                    <div className="flex items-center space-x-2">
                                      <input
                                        type="number"
                                        value={editBonusAmount}
                                        onChange={(e) =>
                                          setEditBonusAmount(e.target.value)
                                        }
                                        className="w-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200/50 dark:border-gray-600/50 rounded-lg px-2 py-1 text-xs font-medium text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:focus:ring-blue-400/50 focus:border-blue-400 dark:focus:border-blue-500 shadow-sm hover:bg-white/90 dark:hover:bg-gray-800/90 hover:shadow-md transition-colors duration-200 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none disabled:opacity-50 disabled:cursor-not-allowed"
                                        step="0.01"
                                        min="0"
                                  ...
                                  <Table.Cell className="px-4 py-3 text-center align-middle">
                                    <div className="flex justify-center items-center h-full">
                                      <CustomButton
                                        ...
                                        size="sm"
                                        onClick={() =>
                                          openBonusDeleteConfirmation(bonus)
                                        }
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-3">
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            Generate notifications for vouchers with clients
                            arriving tomorrow.
                          </p>
                          <CustomButton
                            variant="teal"
                        ...
                        <div className="space-y-3">
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            Generate morning summary of today's arrivals and
                            departures.
                          </p>
                          <CustomButton
                            variant="orange"
            ...
            {activeTab === "ui-management" &&
              (isAdmin || isContentManager || isPublisher) && (
                <div
                  id="ui-management-panel"
                  role="tabpanel"
                  aria-labelledby="tab-ui-management"
                >
                  <Suspense fallback={null}>
                    <UIManagement />
                  </Suspense>
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-1 md:gap-4">
                  <div>
                    <CustomSelect
                      ...
                      onChange={(value) =>
                        setDebtForm({ ...debtForm, officeName: value })
                      }
                      options={[
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-1 md:gap-4">
                  ...
                  <div>
                    <CustomSelect
                      ...
                      onChange={(value) =>
                        setDebtForm({ ...debtForm, currency: value })
                      }
                      options={[
                        { value: "USD", label: "USD ($)" },
                        { value: "EUR", label: "EUR (€)" },
              ...
              itemName={
                debtToDelete
                  ? `${getCurrencySymbol(
                    ...
                    )}${debtToDelete.amount.toFixed(2)} for ${
                      debtToDelete.officeName
                    }`
                  : ""
              }
