...
const AboutHeroSection = () => {
  ...
  // Fetch active hero from API with language parameter
  useEffect(() => {
    const fetchHero = async () => {
      try {
        ...
        const response = await fetch(
          `/api/about-hero/active?lang=${currentLang}`
        );
        ...
        setHero({
          ...
          description:
            "Learn more about our company, our mission, and what makes us special.",
          image: {
  ...
  return (
    <div className="relative w-full h-[40vh] md:h-[50vh] lg:h-[60vh] overflow-hidden">
      ...
      >
        <div
          className={`max-w-4xl ${getTextColorClasses(
            hero.textColor
          )} z-10 ${getTextPositionClasses(hero.textPosition)}`}
        >
          {/* Title */}
          ...
          {hero.description && (
            <p
              className={`text-base sm:text-lg md:text-xl lg:text-2xl opacity-80 leading-relaxed ${
                hero.textPosition === "center"
                  ? "mx-auto max-w-2xl"
                  : hero.textPosition === "right"
                  ? "ml-auto max-w-2xl"
                  : "max-w-2xl"
