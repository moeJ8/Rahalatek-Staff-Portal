...
import { FaPlane, FaMapMarkerAlt, FaClock, FaUser, FaBed, FaUtensils, FaTimes, FaChevronDown, FaChevronUp, FaSearch, FaCrown, FaUsers, FaDollarSign } from 'react-icons/fa';
...
export default function PackageDayBlock({ 
    ...
    const [showDescriptionTranslations, setShowDescriptionTranslations] = useState(false);
    const [showActivitiesTranslations, setShowActivitiesTranslations] = useState(false);
    ...
    // Determine day type
    const dayType = day.isArrivalDay ? 'arrival' 
                  : day.isDepartureDay ? 'departure'
                  : day.isRestDay ? 'rest'
                  : day.tourInfo ? 'tour'
                  : '';
    ...
    const handleDayTypeChange = (newType) => {
        ...
        if (newType === 'arrival') {
            ...
            updates.description = 'Welcome to your journey! Upon arrival at the airport, you will be greeted by our representative who will assist you with the transfer to your hotel. After checking in, take some time to rest and prepare for the exciting adventures ahead.';
            updates.activities = [
            ...
            updates.description = 'Time to say goodbye! After breakfast and hotel check-out, you will be transferred to the airport for your departure flight. We hope you had an amazing journey and look forward to welcoming you again.';
            updates.activities = [
                'Hotel check-out',
                'Transfer to airport',
            ...
            updates.description = 'Enjoy a relaxing day at your own pace. This is your time to explore the area independently, relax at the hotel, or participate in optional activities.';
            updates.activities = [];
            updates.translations = {
                title: {
    ...
    const handleRemoveActivity = (index) => {
        ...
        if (newTranslations.activities && newTranslations.activities.length > index) {
            newTranslations.activities = newTranslations.activities.filter((_, i) => i !== index);
        }
    ...
    return (
        <>
        <Card className={`${getCardColor()} border-2 transition-all duration-300 hover:shadow-lg`}>
            {/* Day Header */}
            ...
            <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                    ...
                    {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && (
                        <button
                            type="button"
                        ...
                        >
                            {showTitleTranslations ? <FaChevronUp className="w-3 h-3" /> : <FaChevronDown className="w-3 h-3" />}
                            Translations
                        </button>
                    )}
                </div>
                <TextInput
                    ...
                    placeholder={`e.g., Day ${dayNumber}: ${dayType === 'arrival' ? 'Arrival & Check-in' : dayType === 'departure' ? 'Departure' : dayType === 'rest' ? 'Free Time' : 'Tour Activity'}`}
                />
                
                {/* Title Translations (only for arrival/departure/rest days) */}
                {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && showTitleTranslations && (
                    <div className="mt-2 space-y-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        <div>
                            <Label value="Arabic (العربية)" className="dark:text-white mb-1 text-xs" />
                            <TextInput
                        ...
                        <div>
                            <Label value="French (Français)" className="dark:text-white mb-1 text-xs" />
                            <TextInput
            ...
            <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                    ...
                    {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && (
                        <button
                            type="button"
                            onClick={() => setShowDescriptionTranslations(!showDescriptionTranslations)}
                            className="text-xs text-blue-600 dark:text-yellow-400 hover:underline flex items-center gap-1"
                        >
                            {showDescriptionTranslations ? <FaChevronUp className="w-3 h-3" /> : <FaChevronDown className="w-3 h-3" />}
                            Translations
                        </button>
                    )}
                </div>
                ...
                {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && showDescriptionTranslations && (
                    <div className="mt-2 space-y-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        <div>
                            <Label value="Arabic (العربية)" className="dark:text-white mb-1 text-xs" />
                            <TextInput
                ...
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && (
                            <button
                                type="button"
                                onClick={() => setShowActivitiesTranslations(!showActivitiesTranslations)}
                                className="text-xs text-blue-600 dark:text-yellow-400 hover:underline flex items-center gap-1"
                            >
                                {showActivitiesTranslations ? <FaChevronUp className="w-3 h-3" /> : <FaChevronDown className="w-3 h-3" />}
                                Translations
                            </button>
                        )}
                    </div>
                ...
                <div className="space-y-2">
                    {(day.activities || []).map((activity, index) => (
                        <div key={index} className="space-y-2">
                            ...
                            {(dayType === 'arrival' || dayType === 'departure' || dayType === 'rest') && showActivitiesTranslations && (
                                <div className="ml-4 pl-3 border-l-2 border-gray-300 dark:border-gray-600 space-y-2">
                                    <div>
                                        <Label value="Arabic" className="dark:text-white mb-1 text-xs" />
                                        <TextInput
                                            value={day.translations?.activities?.[index]?.ar || ''}
                                            onChange={(e) => {
                                                const newTranslations = { ...(day.translations || {}) };
                                                if (!newTranslations.activities) newTranslations.activities = [];
                                                if (!newTranslations.activities[index]) newTranslations.activities[index] = {};
                                                newTranslations.activities[index].ar = e.target.value;
                                    ...
                                    <div>
                                        <Label value="French" className="dark:text-white mb-1 text-xs" />
                                        <TextInput
                                            value={day.translations?.activities?.[index]?.fr || ''}
                                            onChange={(e) => {
                                                const newTranslations = { ...(day.translations || {}) };
                                                if (!newTranslations.activities) newTranslations.activities = [];
                                                if (!newTranslations.activities[index]) newTranslations.activities[index] = {};
                                                newTranslations.activities[index].fr = e.target.value;
                                                onChange({ ...day, translations: newTranslations });
                                            }}
                ...
                {filteredTours.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Tour Details */}
                                <div className="p-3">
                                    <h4 className={`font-bold text-sm mb-2 line-clamp-2 group-hover:text-purple-600 dark:group-hover:text-yellow-400 transition-colors ${
                                        isSelected ? 'text-purple-600 dark:text-yellow-400' : 'text-gray-900 dark:text-white'
                                    }`}>
                                        {tour.name}
                                    </h4>
                                        ...
                                        {(tour.price > 0 || tour.totalPrice > 0) && (
                                            <div className="flex items-center gap-1 font-semibold text-green-600 dark:text-green-400">
                                                ...
                                                <span className="text-xs font-normal text-gray-500 dark:text-gray-400">
                                                    {tour.tourType === 'Group' ? 'per person' : 'per car'}
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {tour.highlights && tour.highlights.length > 0 && (
                                        <div className="mt-2 text-xs text-gray-600 dark:text-gray-400">
                                            ...
                                            {tour.highlights.length > 2 && ` +${tour.highlights.length - 2} more`}
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                            {tourSearch ? 'No tours match your search' : 'No tours available in selected cities'}
                        </p>
                        {tourSearch && (
                            <CustomButton
