...
const manageVoucherOfficePayment = async (voucher, oldVoucher = null, userId) => {
    try {
        // Only create payment if officeName exists and totalAmount > 0
        if (!voucher.officeName || voucher.totalAmount <= 0) {
            // If there was an old payment, delete it
            if (oldVoucher && oldVoucher.officeName) {
                await OfficePayment.findOneAndDelete({
        ...
        // If this is an update, check if we need to update/replace the payment
        if (oldVoucher) {
            const existingPayment = await OfficePayment.findOne({
                relatedVoucher: voucher._id,
                autoGenerated: true
            });

            if (existingPayment) {
                // If office or amount changed, update the payment
                if (oldVoucher.officeName !== voucher.officeName || 
                    oldVoucher.totalAmount !== voucher.totalAmount ||
                    oldVoucher.currency !== voucher.currency) {
                    
                    await OfficePayment.findByIdAndUpdate(existingPayment._id, {
                        officeName: voucher.officeName,
                        amount: voucher.totalAmount,
                        currency: voucher.currency,
                        notes: `Auto-generated from Voucher #${voucher.voucherNumber} - ${voucher.clientName}`
                    });
                }
                return;
            }
        } else {
            // For new vouchers, check if payment already exists (shouldn't happen, but safety check)
            const existingPayment = await OfficePayment.findOne({
                relatedVoucher: voucher._id,
                autoGenerated: true
            });
        ...
        // Create new payment (starts as pending, requires manual approval)
