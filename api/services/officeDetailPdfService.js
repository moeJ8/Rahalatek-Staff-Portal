...
class OfficeDetailPdfService {
    ...
    static async generateOfficeDetailData(officeName, filters) {
        try {
            console.log(`📊 Generating office detail for ${officeName} with filters:`, filters);
            ...
            const filteredVouchers = this.applyFilters(allVouchers, officeName, filters);
            
            console.log(`✅ Filtered vouchers: ${filteredVouchers.length}`);
            
            // Separate service vouchers and client vouchers
            const { serviceVouchers, clientVouchers } = this.categorizeVouchers(filteredVouchers, officeName);
            
            console.log(`📊 Service vouchers: ${serviceVouchers.length}, Client vouchers: ${clientVouchers.length}`);
    ...
    static applyFilters(vouchers, officeName, filters) {
        return vouchers.filter(voucher => {
            ...
            // Currency filter
            if (filters.currency && filters.currency !== 'ALL' && voucher.currency !== filters.currency) {
                return false;
            }
    ...
    static categorizeVouchers(vouchers, officeName) {
        ...
        console.log(`🔍 Categorizing ${vouchers.length} vouchers for office: ${officeName}`);
        ...
        console.log(`✅ Categorization complete: ${serviceVouchers.length} service, ${clientVouchers.length} client`);
    ...
    static checkIfOfficeProvidedService(voucher, officeName) {
        ...
        const hasTransfer = voucher.transfers?.some(t => t.officeName === officeName);
        const hasFlight = voucher.flights?.some(f => f.officeName === officeName);
        
        let hasTrip = false;
        if (voucher.trips && Array.isArray(voucher.trips)) {
            hasTrip = voucher.trips.some(trip => trip && typeof trip === 'object' && trip.officeName === officeName);
        }
        if (!hasTrip) {
        ...
        if (result || voucher.officeName === officeName) {
            console.log(`🎯 Voucher #${voucher.voucherNumber}: service=${result}, client=${voucher.officeName === officeName}, voucherOffice="${voucher.officeName}"`);
        }
    ...
    static calculateServicePayments(voucher, officeName) {
        ...
        if (voucher.trips && Array.isArray(voucher.trips)) {
            voucher.trips.forEach(trip => {
                if (trip && typeof trip === 'object' && trip.officeName === officeName && trip.price) {
    ...
    static calculateServiceBreakdown(serviceVouchers, officeName, allPayments) {
        ...
        serviceVouchers.forEach(voucher => {
            ...
            const voucherPayments = allPayments.filter(p => 
                p.relatedVoucher && 
    ...
    static async generateOfficeDetailPDF(officeData, user) {
        ...
        try {
            ...
            try {
                ...
                console.error('❌ Primary launch failed, trying fallback:', launchError.message);
            ...
            const pdf = await page.pdf({
                ...
                headerTemplate: `
                    <div style="font-size: 10px; color: #000000; width: 100%; text-align: center; margin: 0; padding: 0;">
                        <span style="font-weight: bold;">Rahalatek - Office Detail Report: ${officeData.officeName}</span>
                    </div>
                `,
                footerTemplate: `
    ...
    static getOfficeDetailHtmlTemplate(officeData, user) {
        const { officeName, filters, serviceBreakdown, clientPayments, totalsByCurrency, generatedAt } = officeData;
        
        const currencySymbols = {
            USD: '$', EUR: '€', GBP: '£', AED: 'د.إ', SAR: 'ر.س', TRY: '₺', EGP: 'ج.م'
        };
        ...
        if (filters.month && filters.month.length > 0 && !filters.month.includes('')) {
            const monthNames = filters.month.map(m => new Date(2024, parseInt(m) - 1).toLocaleString('en', { month: 'short' }));
        ...
        if (filters.arrivalMonth && filters.arrivalMonth.length > 0 && !filters.arrivalMonth.includes('')) {
            const arrivalMonthNames = filters.arrivalMonth.map(m => new Date(2024, parseInt(m) - 1).toLocaleString('en', { month: 'short' }));
            filterChips.push({ label: 'Arrival', value: arrivalMonthNames.join(', ') });
        }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { font-size: 20px; color: #111827; margin: 12px 0 4px; }
...
<body>
    ${logoBase64 ? `<img src="${logoBase64}" class="watermark" alt="Watermark" />` : ''}
    
    <div class="header">
        ${logoBase64 ? `<img src="${logoBase64}" class="logo-img" alt="Rahalatek Logo" />` : ''}
        <div class="logo">Rahalatek Travel</div>
        <h1>Office Detail Report</h1>
        <div class="subtitle">${officeName}</div>
        <div class="subtitle">Generated: ${formatDate(generatedAt)} by ${user.username}</div>
    </div>
    
    ${filterChips.length > 0 ? `
    <div class="filter-info">
        ${filterChips.map(chip => `<strong>${chip.label}:</strong> ${chip.value}`).join(' • ')}
    </div>
    ` : ''}
    ...
    <div class="summary-cards">
        <div class="summary-card highlight">
            ...
            <div class="summary-card-value">${officeData.totalVouchersCount}</div>
        </div>
        <div class="summary-card">
            <div class="summary-card-label">Service Vouchers</div>
            <div class="summary-card-value">${officeData.serviceVouchersCount}</div>
        </div>
        <div class="summary-card">
            <div class="summary-card-label">Client Vouchers</div>
            <div class="summary-card-value">${officeData.clientVouchersCount}</div>
        </div>
        ${Object.keys(totalsByCurrency).length > 0 ? `
        <div class="summary-card">
            <div class="summary-card-label">Currencies</div>
            <div class="summary-card-value text">${sortCurrencies(Object.keys(totalsByCurrency)).join(', ')}</div>
        </div>
        ` : ''}
    </div>
    ...
    ${sortCurrencies(Object.keys(servicesByCurrency)).map(currency => `
        <div class="currency-group">
            ...
            <table>
                ...
                <tbody>
                    ${servicesByCurrency[currency].map(item => `
                        <tr>
                            <td>#${item.voucherNumber}</td>
                            <td>${item.clientName}</td>
                            <td>${formatDate(item.createdAt)}</td>
                            <td>${formatDate(item.arrivalDate)}</td>
                            <td class="number">${formatCurrency(item.hotels, currency)}</td>
                            <td class="number">${formatCurrency(item.transfers, currency)}</td>
                            <td class="number">${formatCurrency(item.trips, currency)}</td>
                            <td class="number">${formatCurrency(item.flights, currency)}</td>
                            <td class="number"><strong>${formatCurrency(item.total, currency)}</strong></td>
                    ...
                    <tr class="total-row">
                        ...
                        <td class="number">${formatCurrency(
                            servicesByCurrency[currency].reduce((sum, item) => sum + item.total, 0), 
                            currency
                        )}</td>
                        <td class="number ${
                            servicesByCurrency[currency].reduce((sum, item) => sum + item.remaining, 0) > 0 
                            ? 'negative' : 'positive'
                        }">${formatCurrency(
                            servicesByCurrency[currency].reduce((sum, item) => sum + item.remaining, 0), 
                            currency
                        )}</td>
                    </tr>
                </tbody>
    ...
    ${sortCurrencies(Object.keys(clientsByCurrency)).map(currency => {
        ...
        <div class="currency-group">
            ...
            <table>
                ...
                <tbody>
                    ${clientsByCurrency[currency].map(item => `
                        <tr>
                            <td>#${item.voucherNumber}</td>
                            <td>${item.clientName}</td>
                            <td>${formatDate(item.createdAt)}</td>
    ...
    <div class="summary-box">
        ...
        ${sortCurrencies(Object.keys(totalsByCurrency)).map(currency => `
            <div style="margin-bottom: 16px;">
                ...
                <div class="summary-item">
                    ...
                    <span class="negative">${formatCurrency(totalsByCurrency[currency].serviceTotal, currency)}</span>
                </div>
                <div class="summary-item">
                    <span>Services Remaining Balance:</span>
                    <span class="${totalsByCurrency[currency].serviceRemaining > 0 ? 'negative' : 'positive'}">
                        ${formatCurrency(totalsByCurrency[currency].serviceRemaining, currency)}
                    </span>
                </div>
                ` : ''}
                ...
                <div class="summary-item">
                    ...
                    <span class="positive">${formatCurrency(totalsByCurrency[currency].clientTotal, currency)}</span>
                </div>
                <div class="summary-item">
                    <span>Client Remaining Balance:</span>
                    <span class="${totalsByCurrency[currency].clientRemaining > 0 ? 'negative' : 'positive'}">
                        ${formatCurrency(totalsByCurrency[currency].clientRemaining, currency)}
                    </span>
                </div>
                ` : ''}
                <div class="summary-item">
                    ...
                    <span class="${
                        (totalsByCurrency[currency].clientRemaining - totalsByCurrency[currency].serviceRemaining) > 0 
                        ? 'positive' : 'negative'
                    }">
                        ${formatCurrency(
                            totalsByCurrency[currency].clientRemaining - totalsByCurrency[currency].serviceRemaining, 
                            currency
                        )}
                    </span>
                </div>
